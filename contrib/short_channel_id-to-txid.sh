...
then
    ...
    echo Uses palladium-cli to extract the actual txid >&2
    exit 1
fi
...
BCLI="docker exec palladium-node palladium-cli"
$BCLI getblock "$($BCLI getblockhash "$BLOCK")" true | grep '^    "' | head -n "$((TXNUM + 1))" | tail -n 1 | tr -dc '0-9a-f\n'